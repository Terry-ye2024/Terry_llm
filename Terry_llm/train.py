...
eval_iters = 200
eval_interval = 1000 # 每n步eval和保存checkpoint一次
# 学习率衰减
learning_rate = 6e-4
warmup_iters = 2000
lr_decay_iters = 8000
...
if init_from == 'scratch': 
    print("从头训练模型")
    model_args['vocab_size'] = 50257 # gpt2 tokenizer词表大小
...
while(True):
    ...
    if iter_num>0 and iter_num % eval_interval == 0:
        ...
        checkpoint = {
            ...
            'optimizer_zero_grad': lambda: optimizer.zero_grad(),  # Add this line
            'model_args': model_args,
            'iter_num': iter_num,
            'best_val_loss': best_val_loss
        }
        torch.save(checkpoint, os.path.join(checkpoint_save_dir, 'checkpoint.pt'), pickle_module=dill)
